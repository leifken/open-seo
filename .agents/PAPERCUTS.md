...
- [ ] `2026-09-15T16:05:00Z` — `claude` — `pnpm dev:node` (vite dev with `vite.config.node.ts`) dies during dependency optimization with `No loader is configured for ".md" files: src/server/features/onboarding/openseo-fact-sheet.md?raw` (esbuild pre-bundling hits the `?raw` import in `OnboardingChatAgent.ts`), so the Node runtime cannot be exercised in dev mode; `pnpm build:node && pnpm serve:node` works. Worth excluding that import from `optimizeDeps` in the Node config or adding an esbuild `.md` loader there.
- [ ] `2026-09-15T14:58:00Z` — `claude` — `pnpm test -- <pattern>` (the `test` script is `vitest run`) does not scope the run: pnpm rewrites it to `vitest run -- <pattern>`, and vitest treats the literal `--` as part of its filter args, so the pattern never filters and the full 134-file suite runs every time. Reproduced with `pnpm test -- ai-visibility-tools` (ran all files) vs. `npx vitest run src/server/mcp/tools/ai-visibility-tools.test.ts` (ran just that file, as expected). Worth a `test:file` script wrapping `vitest run` without the extra `--`, or a note in AGENTS.md to use `npx vitest run <path>` directly when iterating on one file.

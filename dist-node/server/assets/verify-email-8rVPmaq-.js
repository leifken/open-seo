...
import { R as Route, n as normalizeAuthRedirect, u as useSession, c as captureClientEvent, A as AuthPageShell, a as AuthPageCard, g as getSignInSearch, b as authClient } from "./router-CFUJAOTG.js";
import { aC as isHostedClientAuthMode, aD as isEmailVerificationBypassed } from "../entry.js";
import { z } from "zod";
import "lucide-react";
...
function VerifyEmailPage() {
  ...
  const isRedirecting = isVerified || Boolean(session?.user?.id);
  ...
  useEffect(() => {
    if (isPending || !isVerified && !session?.user?.id) {
      return;
    }
    if (isVerified) {
